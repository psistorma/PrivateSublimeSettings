...
namespace Cyotek.Windows.Forms
{
  ...
  public partial class ColorPickerDialog : Form
  {
    ...
    protected override void OnLoad(EventArgs e)
    {
      ...
      foreach (ColorPalette palType in Enum.GetValues(typeof(ColorPalette)))
        if (palType != ColorPalette.None)
          palTypeCBox.Items.Add(palType.ToString());

      palTypeCBox.SelectedIndex = 0;
          ...
          try
          {
            ...
            if (serializer != null)
            {
              ...
              if (palette != null)
              {
                ...
                File.Copy(dialog.FileName, sRecentFile, true);
              }
            }
            else
            {
    ...
    {
      if (palTypeCBox.SelectedIndex == -1)
        return;

      ColorPalette selPalType = (ColorPalette)Enum.Parse(typeof(ColorPalette), palTypeCBox.Items[palTypeCBox.SelectedIndex].ToString(), true);
      colorGrid.Colors = ColorPalettes.GetPalette(selPalType);
    }
    private void palTypeCBox_SelectedIndexChanged(object sender, EventArgs e)
    {
      LoadColors();
    }
  }
